...
function ChatHeader({
  ...
  return (
    <div className="relative flex items-center justify-between border-b border px-2.5 pb-[10px] pt-[13px] dark:border">
      <TooltipProvider delayDuration={0}>
        <Tooltip>
          <TooltipTrigger asChild>
            <Button onClick={onClose} variant="ghost" className="md:h-fit md:px-2">
              <X className="dark:text-muted-foreground text-muted-foreground" />
          ...
          <TooltipProvider delayDuration={0}>
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <ArrowsPointingIn className="dark:fill-current fill-current" />
            ...
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    <Expand className="dark:text-muted-foreground text-muted-foreground" />
                    <span className="sr-only">Toggle view mode</span>
                  </Button>
                </TooltipTrigger>
            ...
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    {isPopup ? (
                      <PanelLeftOpen className="dark:fill-current fill-current" />
                    ) : (
                      <Phone className="dark:fill-current fill-current" />
                    )}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button onClick={onNewChat} variant="ghost" className="md:h-fit md:px-2">
                <Plus className="dark:text-muted-foreground text-muted-foreground" />
...
function AISidebar({ className }: AISidebarProps) {
  ...
  const chatState = useAgentChat({
    ...
    onError(error) {
      console.error('[AI Chat] Error in useChat:', {
        error: error.message,
        stack: error.stack,
        name: error.name,
        threadId: threadId ?? undefined,
        ...
        lastMessage: chatState.messages?.[chatState.messages.length - 1]?.role ?? 'none'
      });
    ...
    onResponse: (response) => {
      ...
      if (!response.ok) {
        console.error('[AI Chat] Response not ok:', response.status, response.statusText);
      ...
      {open && (
        <>
          ...
          {isSidebar && !isFullScreen && (
            <>
              ...
              <ResizablePanel
                ...
                minSize={20}
                maxSize={30}
                className="bg-card dark:bg-card mb-1 mr-1 hidden h-[calc(100dvh-8px)] border shadow-sm md:block md:rounded-2xl md:border md:shadow-sm dark:border"
                onResize={(size) => {
                  try {
                    console.debug('AI sidebar panel resized:', size);
                  } catch (error) {
                    console.warn('AI sidebar resize error (safely ignored):', error);
                  }
                }}
              >
          ...
          <div
            tabIndex={0}
            className={cn(
              'fixed inset-0 z-50 flex items-center justify-center bg-transparent p-4 opacity-100 backdrop-blur-sm transition-opacity duration-150 sm:inset-auto sm:bottom-4 sm:right-4 sm:flex-col sm:items-end sm:justify-end sm:p-0',
              'md:hidden',
          ...
          >
            <div
              className={cn(
                'bg-card dark:bg-card w-full overflow-hidden rounded-2xl border border shadow-lg dark:border',
                'md:hidden',
