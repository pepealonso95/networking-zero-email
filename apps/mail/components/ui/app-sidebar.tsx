    ...
    <div>
      {!isFullScreen && (
        <Sidebar
          collapsible="icon"
          {...props}
          className="flex h-screen select-none flex-col items-center pb-2"
        >
          <SidebarHeader
            className={`relative top-2.5 flex flex-col gap-2 ${state === 'collapsed' ? 'px-2' : 'md:px-4'}`}
          >
            {/* Credit Link - only show when sidebar is expanded */}
            {state !== 'collapsed' && (
              <div className="mb-2">
          ...
          {!isLoading && !isPro && showUpgrade && state !== 'collapsed' && (
            <div className="relative top-3 mx-3 mb-4 border bg-card px-4 py-4 backdrop-blur-sm">
              <Button
                  variant="ghost"
                  size="icon"
                  className="absolute right-2 top-2 h-6 w-6 hover:bg-white/10 [&>svg]:h-2.5 [&>svg]:w-2.5"
                  onClick={() => {
              ...
              <button
                ...
                className="mt-3 inline-flex h-7 w-full items-center justify-center gap-0.5 overflow-hidden bg-[#8B5CF6] px-2"
              >
  ...
  return (
    <Dialog open={!!dialogOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogTrigger asChild>
        <button className="inline-flex h-8 w-full items-center justify-center gap-1 self-stretch overflow-hidden rounded-md border-2 border-border bg-card text-foreground shadow-sm hover:bg-accent hover:text-accent-foreground transition-colors">
