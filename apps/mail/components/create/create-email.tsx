...
export function CreateEmail({
  ...
  const handleCloseCompose = async () => {
    await setIsComposeOpen(null);
    navigate('/mail');
  };

  const handleBackgroundClick = (e: React.MouseEvent) => {
    // Only close if clicking on the background container itself, not on child elements
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-1" onClick={handleBackgroundClick}>
      <div className="flex w-[750px] justify-start">
        <button className="flex items-center gap-1 rounded-lg bg-muted hover:bg-muted/80 px-2 py-1.5" onClick={handleCloseCompose}>
          <X className="mt-0.5 h-3.5 w-3.5 fill-muted-foreground dark:fill-white" />
      ...
      {isDraftLoading ? (
        <div className="flex h-[600px] w-[750px] items-center justify-center rounded-2xl border bg-card">
