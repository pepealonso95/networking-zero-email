...
const renderThread = (thread: { id: string; title: string; snippet: string }) => {
  ...
  return getThread?.latest ? (
    <div
      onClick={handleClick}
      key={thread.id}
      className="hover:bg-muted/30 dark:hover:bg-muted/30 cursor-pointer rounded-lg"
    >
        ...
        <div className="flex w-full items-center gap-3">
          ...
          <div className="flex w-full flex-col gap-1.5">
            <div className="flex w-full items-center justify-between gap-2">
              <p className="max-w-[20ch] truncate text-sm font-medium text-foreground">
                {getThread.latest?.sender?.name}
              </p>
              <span className="max-w-[180px] truncate text-xs text-muted-foreground dark:text-muted-foreground">
                {getThread.latest.receivedOn ? format(getThread.latest.receivedOn, 'MMMM do') : ''}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="max-w-[220px] truncate text-xs text-muted-foreground dark:text-muted-foreground">
  ...
  return (
    <div className="mt-6 flex w-full flex-col items-center gap-2">
      ...
      <div className="no-scrollbar relative flex w-full justify-center overflow-x-auto">
        <div className="flex gap-4 px-4">
          {firstRowQueries.map((query, index) => (
            <button
              ...
              className="ai-chat-example-button flex-shrink-0 whitespace-nowrap rounded-md p-1 px-2 text-sm"
            >
              {query}
            </button>
          ))}
        ...
        {/* Left mask */}
        <div className="from-card dark:from-card pointer-events-none absolute bottom-0 left-0 top-0 w-12 bg-gradient-to-r to-transparent"></div>
        {/* Right mask */}
        <div className="from-card dark:from-card pointer-events-none absolute bottom-0 right-0 top-0 w-12 bg-gradient-to-l to-transparent"></div>
      </div>
  ...
  return (
    <div className={cn('flex h-full flex-col', isFullScreen ? 'mx-auto max-w-xl' : '')}>
        ...
        <div className="min-h-full space-y-4 px-2 py-4">
          {!messages.length ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <p className="mb-1 mt-2 hidden text-center text-sm font-medium text-gray-900 md:block dark:text-white">
                Ask anything about your emails
              </p>
              <p className="mb-3 text-center text-sm text-gray-600 dark:text-gray-200">
                  ...
                  {textParts.length > 0 && (
                    <p
                      className={cn(
                        ...
                        message.role === 'user'
                          ? 'overflow-wrap-anywhere text-gray-800 dark:text-gray-100 ml-auto break-words bg-[#f0f0f0] px-2 py-1 dark:bg-[#252525]'
                          : 'overflow-wrap-anywhere mr-auto break-words p-2 text-gray-900 dark:text-white',
                      )}
                    >
                      {textParts.map(
          ...
          {(status === 'submitted' || status === 'streaming') && (
            <div className="flex flex-col gap-2 rounded-lg">
              <div className="flex items-center gap-2">
                <TextShimmer className="text-gray-700 dark:text-gray-200 text-sm">
                  zero is thinking...
                </TextShimmer>
              </div>
            </div>
      ...
      <div className={cn('mb-4 flex-shrink-0 px-4', isFullScreen ? 'px-0' : '')}>
        <div className="ai-chat-container relative rounded-lg p-2">
          {showVoiceChat ? (
            ...
            <div className="flex flex-col">
              <div className="w-full">
                <form id="ai-chat-form" onSubmit={onSubmit} className="relative">
                  <div className="ai-chat-input-area grow self-stretch overflow-y-auto outline-white/5 rounded-md p-2 border border-gray-200 dark:border-gray-500">
                    <div
                      onClick={() => {
              ...
              <div className="grid">
                <div className="flex justify-end">
                  ...
                  >
                    <div className="flex h-5 items-center justify-center gap-1 rounded-sm bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700 px-2 pr-1">
                      <CurvedArrow className="mt-1.5 h-4 w-4 fill-white" />
                    </div>
                  </button>
                </div>
              </div>
        ...
        <div className="mt-1 flex items-center justify-end relative z-10">
          ...
          >
            <SelectTrigger className="flex h-6 w-fit cursor-pointer items-center justify-between gap-1 border-0 dark:bg-card px-2 text-xs hover:bg-[#1E1E1E]">
              <div className="flex items-center gap-1.5 w-full">
                <Puzzle className="h-3.5 w-3.5 fill-white dark:fill-muted-foreground" />
              </div>
