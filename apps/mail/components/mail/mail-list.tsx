      ...
      latestMessage && getThreadData ? (
        ...
        >
          <div
            ...
            key={idToUse}
            className={cn(
              'hover:bg-muted hover:bg-primary/5 group relative mx-1 flex cursor-pointer flex-col items-start rounded-lg py-2 text-left text-sm transition-all hover:opacity-100',
          ...
          >
            <div
              className={cn(
                'dark:bg-card absolute right-2 z-[25] flex -translate-y-1/2 items-center gap-1 rounded-xl border bg-white p-1 opacity-0 shadow-sm group-hover:opacity-100',
                index === 0 ? 'top-4' : 'top-[-1]',
              )}
            >
              <Tooltip>
              ...
              <Tooltip>
                ...
                <TooltipContent
                  side={index === 0 ? 'bottom' : 'top'}
                  className="dark:bg-card mb-1 bg-white"
                >
            ...
            <div className="flex w-full items-center justify-between gap-4 px-4">
              <div>
                <Avatar className="h-8 w-8 rounded-full border dark:border-none">
                  ...
                  {isGroupThread ? (
                    <div className="flex h-full w-full items-center justify-center rounded-full bg-background p-2 dark:bg-muted">
                      <GroupPeople className="h-4 w-4" />
                    </div>
                  ) : (
                    <>
                      <AvatarImage
                        className="rounded-full bg-background dark:bg-muted"
                ...
                <div className="w-full">
                  <div className="flex w-full flex-row items-center justify-between">
                    ...
                    {latestMessage.receivedOn ? (
                      <p
                        className={cn(
                          'text-muted-foreground text-nowrap text-xs font-normal opacity-70 transition-opacity group-hover:opacity-100 dark:text-muted-foreground',
                  ...
                  <div className="flex justify-between">
                    {isFolderSent ? (
                      <p
                        className={cn(
                          'mt-1 line-clamp-1 max-w-[50ch] overflow-hidden text-sm text-muted-foreground md:max-w-[25ch]',
                        )}
                      >
                      ...
                      <p
                        className={cn(
                          'mt-1 line-clamp-1 w-[95%] min-w-0 overflow-hidden text-sm text-muted-foreground',
                        )}
                      >
                        {highlightText(latestMessage.subject, searchValue.highlight)}
                      </p>
  ...
  return (
    <div className="select-none py-1" onClick={handleMailClick}>
      <div
        key={message.id}
        className={cn(
          'hover:bg-muted hover:bg-primary/5 group relative mx-[8px] flex cursor-pointer flex-col items-start overflow-clip rounded-[10px] border-transparent py-3 text-left text-sm transition-all hover:opacity-100',
        )}
      >
        <div
          className={cn(
          ...
          <div className="flex w-full justify-between">
            <div className="w-full">
              ...
              <div className="flex justify-between">
                <p
                  className={cn(
                    'mt-1 line-clamp-1 max-w-[50ch] text-sm text-muted-foreground md:max-w-[30ch]',
                  )}
                >
                  {draft?.subject}
                </p>
          ...
          if (label.name.toLowerCase() === 'notes') {
            return (
              <Tooltip key={label.id}>
                <TooltipTrigger asChild>
                  <Badge className="rounded-md bg-blue-100 p-1 text-blue-700 hover:bg-blue-200 dark:bg-blue-900/30 dark:text-blue-400">
...
function getLabelIcon(label: string) {
  ...
  switch (normalizedLabel) {
    case 'starred':
      return <Star className="h-3.5 w-3.5 fill-blue-400 stroke-blue-400" />;
    default:
      return null;
  }
}
