...
function SortableNote({
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        'group relative mb-3 overflow-hidden rounded-md border border p-3 dark:border',
        note.isPinned && 'ring-1 ring-blue-200 dark:ring-blue-800',
      ...
      <div className="flex items-start gap-3 pl-1.5">
        ...
        <div className="flex items-center">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="dark:bg-card border bg-card dark:border"
            >
              <DropdownMenuItem
                onClick={onEdit}
                className="text-black focus:bg-white focus:text-foreground/90 dark:focus:bg-card dark:focus:text-white"
              >
              ...
              <DropdownMenuSub>
                <DropdownMenuSubTrigger className="text-black focus:bg-white focus:text-foreground/90 dark:focus:bg-card dark:focus:text-white">
                  <PaintBucket className="mr-2 h-4 w-4" />
                  <span>{t('common.notes.actions.changeColor')}</span>
                </DropdownMenuSubTrigger>
                <DropdownMenuPortal>
                  <DropdownMenuSubContent className="dark:bg-card w-48 border bg-card dark:border">
...
export function NotesPanel({ threadId }: NotesPanelProps) {
  ...
  return (
    <div className="relative" ref={panelRef}>
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            size="sm"
            className={cn(
              'inline-flex h-7 w-7 items-center justify-center gap-1 overflow-hidden rounded-md bg-white dark:bg-muted',
                              notes.length > 0 && 'text-blue-500',
              isOpen && 'bg-white/80 dark:bg-muted/80',
            )}
          ...
          >
            <StickyNote
              className={cn(
                'h-4 w-4',
                notes.length > 0 ? 'fill-blue-200 dark:fill-blue-900' : 'text-muted-foreground',
              )}
            />
            {notes.length > 0 && (
        ...
        <TooltipContent side="bottom" className="bg-white dark:bg-muted">
      ...
      {isOpen && (
        <div
          className="animate-in fade-in-20 zoom-in-95 dark:bg-card fixed top-[5rem] z-50 h-[calc(100dvh-5rem)] max-h-[calc(100dvh-5rem)] w-full max-w-[100vw] overflow-hidden rounded-t-lg border border-t bg-card shadow-lg duration-100 sm:absolute sm:right-0 sm:top-full sm:mt-2 sm:h-auto sm:max-h-[80vh] sm:w-[350px] sm:max-w-[90vw] sm:rounded-xl sm:border lg:left-[-200px] xl:left-[-300px] dark:border"
          onClick={handlePanelClick}
        >
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border p-3 dark:border">
            ...
            <div className="sticky top-[49px] z-10 px-3 pb-0 pt-2">
              <div className="relative">
                ...
                <Input
                  ...
                  className="border bg-white pl-8 text-sm text-black placeholder:text-muted-foreground focus:outline-none dark:border dark:bg-card dark:text-white"
                />
              </div>
            </div>
          )}
          ...
          <div className="flex h-full flex-col sm:max-h-[calc(80vh-100px)]">
            ...
            >
                ...
                <div className="p-3">
                  {notes.length === 0 && !isAddingNewNote ? (
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      <StickyNote className="mb-2 h-12 w-12 text-muted-foreground opacity-50" />
                      <p className="text-sm text-foreground/90">
                        {t('common.notes.empty')}
                      </p>
                      <p className="mb-4 mt-1 max-w-[80%] text-xs text-muted-foreground">
                    ...
                    <>
                      {searchQuery && filteredNotes.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-6 text-center">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            className="mt-4 border bg-white text-black dark:border dark:bg-muted text-foreground"
                      ...
                      {isAddingNewNote && (
                        <div className="relative mb-3 overflow-hidden rounded-md border bg-background dark:border dark:bg-card">
                          <div
                            className={cn(
                              'absolute bottom-0 left-0 top-0 w-1.5 border-l-4',
                          ...
                          <div className="">
                            <Textarea
                              ...
                              className="min-h-[20px] resize-none border-none bg-transparent text-black focus:outline-none text-foreground"
                                      ...
                                      <TooltipTrigger asChild>
                                        <button
                                          ...
                                          className={cn(
                                            ...
                                            color.value === 'lightblue' ? 'bg-sky-500' : '',
                            ...
                            <div className="mx-1 my-2 flex justify-between">
                              <Button
                                variant="ghost"
                                size="sm"
                                className="text-muted-foreground hover:bg-white/10 hover:text-[#a0a0a0]"
                                onClick={() => {
                                  setIsAddingNewNote(false);
                                  setNewNoteContent('');
                                  setSelectedColor('default');
                      ...
                      {!isAddingNewNote && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="mt-1 w-full border bg-white/5 hover:bg-white/10 dark:border text-foreground"
                          onClick={() => setIsAddingNewNote(true)}
                        >
              ...
              <DragOverlay>
                {activeId ? (
                  <div className="rounded-md border border bg-white p-3 pl-7 shadow-md dark:border dark:bg-card">
            ...
            {editingNoteId && (
              <div className="dark:bg-card border-t border bg-card p-3 dark:border">
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    className="min-h-[100px] resize-none border bg-background text-sm text-black dark:border dark:bg-card text-foreground"
                    placeholder={t('common.notes.addYourNote')}
                  />
