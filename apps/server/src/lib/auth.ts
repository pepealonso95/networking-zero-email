...
const connectionHandlerHook = async (account: Account) => {
  ...
  const logMessage = `🔗 connectionHandlerHook called with account: ${JSON.stringify({
  ...
  console.log('💾 Attempting to save connection:', {
    ...
    providerId: connectionRecord.providerId,
  });
  ...
  return betterAuth({
    ...
    hooks: {
      after: createAuthMiddleware(async (ctx) => {
        // Handle account linking for adding additional email connections
        const hookLogMessage = `🔍 Hook triggered for path: ${JSON.stringify({
          path: ctx.path,
          method: ctx.method,
        ...
        if (ctx.path.includes('/link-social') || ctx.path.includes('/callback/') || ctx.path.includes('/api/auth/')) {
          const session = ctx.context.session;
          const account = ctx.context.account;
          
          console.log('🔗 Social link/callback detected:', {
            path: ctx.path,
            hasSession: !!session,
            hasAccount: !!account,
            accountId: account?.id,
          ...
          // If we have both session and account, this is likely an account linking
          if (session?.user && account && (account.accessToken || account.refreshToken)) {
            console.log('🔗 Triggering connection handler for account linking');
            try {
