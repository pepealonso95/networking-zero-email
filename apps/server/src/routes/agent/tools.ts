...
const askZeroMailbox = (connectionId: string) =>
  tool({
    ...
    execute: async ({ question, topK = 3 }) => {
      // Return mock data in development mode to avoid Cloudflare auth issues
      if (env.NODE_ENV === 'development') {
        console.log('[Development Mode] askZeroMailbox called with question:', question);
        return {
          response: [
            "I don't have access to your email data in development mode. Please use other tools like 'listThreads' to browse your emails.",
            "This feature requires Cloudflare Vectorize which is not available in local development.",
            "You can still use other email management tools to organize your inbox."
          ],
          success: false,
        };
      }
...
const askZeroThread = (connectionId: string) =>
  tool({
    ...
    execute: async ({ threadId, question }) => {
      // Return mock data in development mode to avoid Cloudflare auth issues
      if (env.NODE_ENV === 'development') {
        console.log('[Development Mode] askZeroThread called with threadId:', threadId, 'question:', question);
        return {
          response: "I don't have access to thread-specific data in development mode. Please use the 'getThread' tool to view the full thread content.",
          success: false,
        };
      }
...
const composeEmailTool = (connectionId: string) =>
  tool({
    ...
    execute: async (data) => {
      try {
        console.log('[ComposeEmail Tool] Starting email composition with data:', {
          prompt: data.prompt?.substring(0, 100) + '...',
          emailSubject: data.emailSubject,
          to: data.to,
          cc: data.cc,
          threadMessagesCount: data.threadMessages?.length ?? 0,
          connectionId
        });
        ...
        console.log('[ComposeEmail Tool] Email composition successful:', {
          bodyLength: newBody?.length ?? 0,
          bodyPreview: newBody?.substring(0, 100) + '...'
        });
        
        return { newBody };
      } catch (error) {
        console.error('[ComposeEmail Tool] Error composing email:', {
        ...
        throw new Error('Failed to compose email: ' + (error instanceof Error ? error.message : String(error)));
      }
    },
  });
